...
- It is called after the component is mounted (all functions and hooks executed)
- Ideal for API calls
- Lazy initializations are great for perfomance. You just pass a function to the useState and it will be called once the component has been rendered for THE FIRST TIME (when the default value is needed).
- The dependency array in the useEffect is vital to get performance. A component can be rerendered by a parent, but we only run that funcion if one of the elements in the array changes
- If you don't add anything to the dependency array then it is run just ONCE
- Dependency array uses shallow comparison, I can have A and B the same objects, but it will re render because the objects are different
- React uses Object.is for the comparison in the dependency array
- A custom hook should start with use. But is a custom hook if it uses hooks internally.
- useRef doesn't trigger rerenders. It is set on the first render.
