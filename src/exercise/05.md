...
- Once a component is rendered is considered mounted
- Direct modifications of the DOM should be done in the effects because the DOM tree is already rendered
- We need refs to access to them because is in the ReactDOM.render where the magic happens, not in the React side
- Be sure to delete event handlers on unmount
- In React we don't work with DOM we work with UI Descriptors that React uses later to create nodes
- Refs are mutable, are used for referencing things without triggering rerenders. If we want to trigger rerenders it is better to use useState
- Return of the useEffect is a cleanup === componentWillUnmount for classes
